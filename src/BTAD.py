...
def CreateBtadDataset(log_file):
    try:
        ...
        myPrint(e, log_file)
        myPrint("Error in CreateBtadDataset function!", log_file)
...
def prepareBtad(log_file):
    if os.path.isdir(btad_folder):
        if (os.path.exists(btad_config_file)):
            ...
            myPrint("Preparing the BTAD Dataset...", log_file)
